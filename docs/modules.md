...
Computes the latest complete 24-hour Asia/Shanghai window, anchored at 06:00.
In GitHub Actions it writes `DIGEST_WINDOW_START`, `DIGEST_WINDOW_END`, and
`DIGEST_SEGMENT_PATH` to `$GITHUB_ENV`. Explicit `--hours 6 --anchor-hour 0`
still supports legacy 6-hour windows.
...
until_time:<unix>` so the scheduled fetch maps exactly to its 24-hour window.
...
- `fetch_many(feeds, kind, max_items, role_template, since, until)` — fetches every feed
  concurrently (capped at 8 workers), filters an optional exact publication
  range before the per-source cap, and tags each item with its `kind`.
...
| `tests/test_archive_data.py`          | Daily-window and legacy segment merge + index                    |
| `tests/test_daily_workflow.py`        | Daily cron and 24-hour workflow configuration                    |
